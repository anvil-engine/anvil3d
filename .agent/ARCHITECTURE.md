...
## Startup (current)
cmdline -> log level -> mount game (gameinfo.txt) -> console -> Vulkan library probe -> window -> render::Device -> devui -> anvil.cfg, +commands (e.g. +map) -> loop (events, free camera, clock, beginFrame/world draw3d/draw2d/endFrame, fps_max) -> shutdown (world, device, window).
