...
| Entity I/O  | PARTIAL (bounded generic outputs and delayed queue; `logic_auto` spawn, `logic_relay`, `logic_timer`, `logic_branch`, `math_counter`, triggers including `trigger_changelevel`, and linear `func_door` dispatch) | NOT STARTED | NOT STARTED |
| Materials   | PARTIAL     | NOT STARTED | NOT STARTED |
| Textures    | PARTIAL (VTF->GPU; no frames/cubemaps/HDR) | NOT STARTED | NOT STARTED |
| World render | PARTIAL (brush world, LightmappedGeneric-style, lightmaps, displacements, PVS/frustum, 2D sky, moving linear `func_door` entities with synced collision, WVT blend, static props with ambient-only light; no 3D sky/water/decals/overlays) | NOT STARTED | NOT STARTED |
...
- Brush entities: static origin/angle placement is verified on HL2 data. Linear `func_door` movement uses authored `lip`, `speed`, `wait`, and `movedir`, updates its render transform and kinematic Jolt hull, and dispatches `OnOpen`, `OnClose`, `OnFullyOpen`, and `OnFullyClosed`; angle rotation (especially roll sign) remains UNVERIFIED. `Lock`/`Unlock` and `SetSpeed` are supported. Rotating doors, tracktrains, render modes, parenting, and other moving brush classes remain unsupported.
- Entity I/O: linear `func_door` supports authored `Open`, `Close`, `Toggle`, `Lock`, `Unlock`, and `SetSpeed` inputs, wait timing, and `OnOpen`, `OnClose`, `OnFullyOpen`, and `OnFullyClosed` outputs. Its moving render mesh and kinematic Jolt hull stay synchronized. Rotating doors and tracktrains remain unsupported.
