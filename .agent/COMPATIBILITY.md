...
| VGUI        | PARTIAL (resources/localization, schemes, panel descriptors, original custom-font rasterization and single-line batching; no panel runtime/command execution) | NOT STARTED | NOT STARTED |
...
- Basic Panel/EditablePanel/Frame, Label, Button and Divider instances now preserve resolved bounds and runtime state. Button focus follows authored positive tab positions, skips hidden/disabled controls, wraps, hit-tests topmost-first and yields the authored command for a later dispatcher. Unknown control classes remain explicit Unsupported instances; no generic substitute is painted.
