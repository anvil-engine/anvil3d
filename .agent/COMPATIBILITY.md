...
| Entity I/O  | PARTIAL (bounded generic outputs and delayed queue; `logic_auto` spawn, `logic_relay`, `logic_timer`, `logic_branch`, `math_counter`, triggers including `trigger_changelevel`, and moving door dispatch) | NOT STARTED | NOT STARTED |
...
- Brush entities: static origin/angle placement is verified on HL2 data. Linear `func_door` movement uses authored `lip`, `speed`, `wait`, and `movedir`; `func_door_rotating` uses authored axis flags, `distance`, `speed`, and `wait`. Both update render transforms and kinematic Jolt hulls; rotation sign remains UNVERIFIED. `func_tracktrain` follows bounded linked `path_track` targets, supports authored speed/height/orientation flags, and updates render transforms plus kinematic Jolt hulls. `Lock`/`Unlock` and `SetSpeed` are supported for doors; common train start/stop inputs are supported. Moving brush render modes, parenting, and other classes remain unsupported.
- Entity I/O: `func_door` and `func_door_rotating` support authored `Open`, `Close`, `Toggle`, `Lock`, `Unlock`, and `SetSpeed` inputs, wait timing, and `OnOpen`, `OnClose`, `OnFullyOpen`, and `OnFullyClosed` outputs. `func_tracktrain` supports `Start`, `Stop`, `Resume`, `Toggle`, and `SetSpeed`, and fires `OnStart`/`OnNextPoint`; each `path_track` fires `OnPass`. Moving render meshes and kinematic Jolt hulls stay synchronized.
