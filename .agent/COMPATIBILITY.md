...
| Entity I/O  | PARTIAL (bounded generic outputs and delayed queue; `logic_auto` spawn, `logic_relay`, `logic_timer`, `logic_branch`, `math_counter`, triggers including `trigger_changelevel`, and moving door/button dispatch) | NOT STARTED | NOT STARTED |
| Materials   | PARTIAL     | NOT STARTED | NOT STARTED |
| Textures    | PARTIAL (VTF->GPU; no frames/cubemaps/HDR) | NOT STARTED | NOT STARTED |
| World render | PARTIAL (brush world, LightmappedGeneric-style, lightmaps, displacements, PVS/frustum, 2D sky, moving `func_door`/`func_door_rotating` entities with synced collision, WVT blend, static props with ambient-only light; no 3D sky/water/decals/overlays) | NOT STARTED | NOT STARTED |
