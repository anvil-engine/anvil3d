...
- Brush entities: static origin/angle placement is verified on HL2 data. Linear `func_door` movement uses authored `lip`, `speed`, `wait`, and `movedir`; `func_door_rotating` uses authored axis flags, `distance`, `speed`, and `wait`. Both update render transforms and kinematic Jolt hulls; rotation sign remains UNVERIFIED. `Lock`/`Unlock` and `SetSpeed` are supported. Tracktrains, render modes, parenting, and other moving brush classes remain unsupported.
- Entity I/O: `func_door` and `func_door_rotating` support authored `Open`, `Close`, `Toggle`, `Lock`, `Unlock`, and `SetSpeed` inputs, wait timing, and `OnOpen`, `OnClose`, `OnFullyOpen`, and `OnFullyClosed` outputs. Moving render meshes and kinematic Jolt hulls stay synchronized. Tracktrains remain unsupported.
