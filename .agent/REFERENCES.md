...
- Nothing is vendored, so `licenses/` does not exist yet. Binary distributions must ship license texts for SDL3 (zlib), volk (MIT), Vulkan-Headers (Apache-2.0/MIT), MoltenVK if bundled (Apache-2.0) and, with devui, ImGui (MIT); add `licenses/` with packaging.
