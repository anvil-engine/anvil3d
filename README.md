...
Development snapshot after 0.10.0. Mounts loose files and VPK archives, runs configs, and renders HL2 BSP maps through Vulkan with textures, lightmaps, displacements, PVS/frustum culling, a 2D skybox, brush entities, and ambient-lit static props. Source model data is now read far enough for LOD0 meshes, skeletons, sequences, inline raw/RLE animation frames, CPU skinning, and diagnostic animated rendering. Bounded entity I/O supports common logic, triggers, movers (including `func_button` and `func_breakable`), `env_fade`, `point_viewcontrol`, `prop_dynamic` behavior, authored humanoid `npc_*` visual entities, and authored `scripted_sequence` animation bridges; PCM WAV, `ambient_generic`, and a bounded `env_soundscape` subset play through SDL3. `UnlitTwoTexture` authored materials and bounded `scenes.image` indexing are supported foundations.

An independent Jolt player supports WASD walking, collision, gravity, jumping, stairs, and precise overlap checks; static props and `prop_physics` can use bounded authored Source PHY compact-convex hulls for compound collision. A free camera remains available through noclip. Original VGUI resources, schemes, localization, fonts, and a limited set of common panels can be inspected and rendered for diagnostics. These systems are compatibility foundations, not a working retail game: client/server DLLs, broader Source sound-script layout and semantics, networking, most gameplay entities, animation blending and many Source rendering features remain unsupported. Studio `.ani` external blocks and sectioned animation data are decoded when the authored file is available; include models, pose blending, flexes and bodygroups remain unsupported. HL2 campaign execution is not yet available.
`scripted_sequence` can schedule or repeat a named animation on a loaded `prop_dynamic` or allowlisted humanoid `npc_*` visual entity, with `BeginSequence`, `CancelSequence`, and lifecycle outputs. `logic_choreographed_scene` parses bounded original VCD files and schedules authored sequence, speak, and trigger events, with `Start`, `Stop`, `Cancel`, and lifecycle outputs. Retail scene image/event semantics and NPC ownership, AI, combat, and other sequence targets remain unsupported. See [.agent/COMPATIBILITY.md](.agent/COMPATIBILITY.md).
`point_viewcontrol` applies an authored origin, angles, and FOV while enabled, with `Enable`, `Disable`, `SetAngles`, and `SetFOV`; target movement and blending remain unsupported.
